...
{
  double Mops, t1[1], t2, t3, t4, x1, x2;
...
#pragma scop    
    ...
    for (k = 1; k <= np; k++) {
      ...
      // This should be considered as a black box by ppcg
      for (i = 1; i <= 100; i++) {
        ik = kk / 2;
...
#pragma endscop
    ...
    /* This atomic OpenMp pragma can be eliminated.  It's used because the 
       external OpenMP pragma is #pragma omp parallel (so that threads are not
       created each iteration, otherwise performance will be horrible).
       So we can insert a simple: if (my_thread_id()==0), so that all the code
       that is not a part of the parallel scop is run atomically.  Check whether
       there is a data shared between the different threads and the master
       thread.  */
    // NQ is a constant = 10
    for (i = 0; i < NQ; i++) {
      #pragma omp atomic
