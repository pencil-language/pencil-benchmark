 ...
 * This function tries first to look for a GPU in the first available platform,
 * if it fails, it tries to use CPU.
 */
cl_device_id opencl_create_device()
{
	...
	if (err < 0) {
		fprintf(stderr, "Could not find a platform.\n");
		exit(1);
	}
	...
	if (err == CL_DEVICE_NOT_FOUND)
		err = clGetDeviceIDs(platform, CL_DEVICE_TYPE_CPU, 1, &dev,
				NULL);
	if (err < 0) {
		fprintf(stderr, "Error while looking for a device.\n");
		exit(1);
	}
	return dev;
}
