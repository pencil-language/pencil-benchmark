...
#endif
    {
        ...
        for ( int n_r = 0; n_r < resampled_rows; n_r++ )
        {
            ...
            for ( int n_c = 0; n_c < resampled_cols; n_c++ )
            {
                ...
                int coord_00_r = clampi( (int)i_r, 0, original_rows - 1 );
                int coord_00_c = clampi( (int)i_c, 0, original_cols - 1 );

                int coord_01_r = coord_00_r;
                int coord_01_c = clampi( coord_00_c + 1, 0, original_cols - 1 );

                int coord_10_r = clampi( coord_00_r + 1, 0, original_rows - 1 );
                int coord_10_c = coord_00_c;

                int coord_11_r = clampi( coord_00_r + 1, 0, original_rows - 1 );
                int coord_11_c = clampi( coord_00_c + 1, 0, original_cols - 1 );
